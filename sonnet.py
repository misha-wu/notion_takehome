...
import click
import textwrap
from constants import __HELP_STR_TEMPLATE, __MAIL_TEMPLATE, __MSG, __RESULTS, __SENDER, DEFAULT_TEXT_WIDTH
...
@click.command()
@click.option('-r', '--recipient', help='Recipient username', required=True)
@click.option("-a", '--all_mail', is_flag=True, show_default=True, default=False, help="Show all mail")
def check_mail(recipient: str, all_mail: bool):
  ...
  try:
    ...
    for result in mail_results:
      ...
      click.echo(textwrap.fill(mail_str,
                               DEFAULT_TEXT_WIDTH,
